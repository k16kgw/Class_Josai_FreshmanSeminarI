...
大学では，欠席連絡，課題の確認，研究室訪問の依頼など，メールで用件を伝える機会が多い．  
メールは「相手の時間を使うコミュニケーション」であるため，結論が分かりやすく，誤解が起きにくい形で書くことが重要である．

### メールの基本原則

- 件名で用件が分かるようにする  
  例：第4回課題の提出方法について（情報数理学科SI26999城西太郎）
- 最初に名乗る  
  所属，学籍番号，氏名を最初に書く．
- 目的と結論を先に書く  
  「何をしてほしいか」「何を確認したいか」を冒頭で述べる．
- 必要情報を漏れなく書く  
  課題名，締切，エラー文，発生時刻など，判断に必要な情報を添える．
- 丁寧だが回りくどくしない  
  長文より，短い段落と箇条書きで整理する．
- 返信しやすい形にする  
  質問は箇条書きにし，Yes／Noで答えられる形にする．
- 送信前に読み返す  
  宛先，添付ファイル，敬称，誤字脱字を確認する．
...
件名は「何の」「どの授業の」「誰から」が分かる形にする．

良い例

- 第4回課題の提出形式について（SI26999城西太郎）
- 欠席連絡：第4回フレッシュマンセミナーI（SI26999城西太郎）
- 面談のお願い（情報数理学科SI26999城西太郎）

### 本文の書き方のコツ

- 1段落は2〜4行を目安にする
- 重要情報は数字で書く  
  例：提出期限，エラー発生時刻，ファイル名など
- 画像やファイルを添付する場合は，本文に必ず書く  
  例：スクリーンショットを1枚添付しました．

### NG例と改善例

NG例

> 先生，課題がうまくいきません．どうすればいいですか．

改善例（必要情報を添える）

> 第4回課題の提出時にエラーが出たため，確認をお願いいたします。  
> WebClassで提出を押すと「ファイル形式が不正です」と表示されます。  
> ファイル名は「第4回_SI26999_城西太郎.docx」で、2026年5月11日21:10に発生しました。  
> エラー画面のスクリーンショットを添付しました。  

### 教員宛メールの例（課題の提出相談）

件名：第4回課題の提出エラーについて（SI26999城西太郎）

本文：

〇〇先生  

  情報数理学科のSI26999　城西太郎です。  
  第4回課題の提出について確認したいことがあり、ご連絡いたしました。  
  WebClassで提出を行う際にエラーが表示され、提出できませんでした。  
  発生時刻は2026年5月11日21:10で、表示内容は「ファイル形式が不正です」です。  
  使用したファイル名は「第4回_SI26999_城西太郎.docx」です。  
  エラー画面のスクリーンショットを添付しますので、原因と対処法をご教示ください。  
...
```{tip} 添付ファイルの注意
送信前に，本文に添付の記載があるかを確認し，実際に添付されているかも確認する．
```
...
1. 前回の講義の課題3の文章をChatGPTに校正してもらい，校正結果を記入せよ．
