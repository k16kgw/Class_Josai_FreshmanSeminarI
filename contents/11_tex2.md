...
数学の文書では，定義・定理・補題などを本文と区別して示す．
このような記述には，`amsthm` パッケージが提供する**定理環境**を使用する．

まずプリアンブルで `amsthm` パッケージを読み込む．

```latex
\usepackage{amsthm}
```

続いて，使用する定理環境をプリアンブルに定義する．
次のコードは `\begin{document}` より前に入力する．
...
- `\theoremstyle{...}`：その後に定義する定理環境の書式を指定する命令

| 指定 | 主な用途 | 標準の書式 |
| --- | --- | --- |
| `plain` | 定理・補題・命題・系・予想 | 見出しは太字，本文の欧文は斜体 |
| `definition` | 定義・仮定 | 見出しは太字，本文は立体 |
| `remark` | 注・注意 | 見出しは斜体，本文は立体 |

定理などの主張と，定義や補足説明とを見分けやすくするために，内容に応じて書式を使い分ける．

### `newtheorem`の引数

最初の定理環境は，次の命令で定義している．

```latex
\newtheorem{thm}{定理}[section]
```

- `{thm}`：ソースコードで使用する環境名．本文では `\begin{thm}` と `\end{thm}` を使用する．
- `{定理}`：PDFに表示される見出し．
- `[section]`：節が変わるたびに定理番号を1から数え直す指定．第4節の最初の定理は「定理 4.1」と表示される．
...
- `{lem}`：ソースコードで使用する環境名．
- `[thm]`：`thm` と同じ番号を共有する指定．
- `{補題}`：PDFに表示される見出し．

| 環境名 | PDFの見出し | 用途 |
| --- | --- | --- |
| `thm` | 定理 | 証明の対象となる主要な主張 |
| `lem` | 補題 | 定理などを証明するために用いる補助的な主張 |
| `prop` | 命題 | 真偽を明確に定められる数学的な主張 |
| `cor` | 系 | すでに示した定理などから直接導かれる主張 |
| `conj` | 予想 | 正しいと考えられているが，まだ証明されていない主張 |
| `dfn` | 定義 | 用語や記号の意味を定める記述 |
| `ass` | 仮定 | 議論の前提として置く条件 |
| `rem` | 注 | 本文を補足する説明 |

````{tip} 使用例
定義した環境は，`\begin{document}` と `\end{document}` の間で使用する．
...
\begin{dfn}\label{dfn:linear-independent}
ベクトル $\boldsymbol{a}_1,\ldots,\boldsymbol{a}_n$ が1次独立であるとは，
\[
  c_1\boldsymbol{a}_1+\cdots+c_n\boldsymbol{a}_n=\boldsymbol{0}
\]
を満たすとき，$c_1=\cdots=c_n=0$ となることをいう．
\end{dfn}

\begin{thm}\label{thm:unique-expression}
1次独立なベクトルの1次結合による表現は一意である．
\end{thm}

定義\ref{dfn:linear-independent}の条件を用いると，
定理\ref{thm:unique-expression}を示すことができる．
```
...
![theorem](./figs/11/theorem.png)

---
````

`\label` の名前には，定義を表す `dfn:`，定理を表す `thm:` などの接頭辞を付けると，参照先を区別しやすい．
番号の参照が `??` と表示された場合は，もう一度タイプセットする．
...
作成したTeXファイルとPDFファイルをそれぞれ，WebClassの<span style="color: red; ">「第11回課題」問1・2</span>から提出せよ．

<img src="./figs/linearalgebra.png" width="100">

![image](./figs/11/linearalgebra.png)

![image](./figs/11/typeset.png)
...
<span style="color: red; ">7月5日(日)23:59まで</span>
...
````{note} 追加演習（自由提出・加点あり）
下記手順2〜4をmac内のスクリーンレコーダー（⌘+5）で動画として撮影し，「第11回課題」問3から提出せよ．
